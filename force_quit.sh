...
collect_descendants() {
  local root="$1"
  local visited="/tmp/fq.visited.$$"
  local queue="/tmp/fq.queue.$$"
  : >"$visited"; : >"$queue"; echo "$root" >>"$queue"

  while IFS= read -r line; do
    echo "$line"
  done < <(build_ps_table) >"/tmp/fq.ps.$$"
  ...
  while IFS= read -r current || [[ -n "${current-}" ]]; do
    ...
    local comm
    comm=$(/usr/bin/awk -v P="$current" '$1==P{ $1="";$2=""; sub(/^ /,""); print }' "/tmp/fq.ps.$$")
    if in_ignore "$comm"; then
      continue
    fi

    /usr/bin/awk -v P="$current" '{ if ($2==P) print $1 }' "/tmp/fq.ps.$$" >>"$queue"
  done <"$queue"

  cat "$visited"

  /bin/rm -f "$visited" "$queue" "/tmp/fq.ps.$$" 2>/dev/null || true
}

# Compute ancestry depth for PIDs to order children before parents (post-order)
compute_depths() {
  # stdin: "PID" lines; stdout: "depth PID"
  local tmp="/tmp/fq.ps2.$$"
  build_ps_table >"$tmp"
