...
# Cleaner multi-stack con auto-detección: Java (Maven/Gradle), Node.js/TS y Python.
# Uso:
...
#   Java: pom.xml, mvnw, build.gradle, gradlew
...
if contains java; then
  if [ -f "$REPO_ROOT/pom.xml" ]; then
    ...
    $DRY_RUN && echo "DRY-RUN mvn -f '$REPO_ROOT/pom.xml' clean -Dmaven.clean.failOnError=false" \
             || mvn -f "$REPO_ROOT/pom.xml" clean -Dmaven.clean.failOnError=false || true
  fi
