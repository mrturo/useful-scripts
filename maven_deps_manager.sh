...
if [ ${#REPOS_TO_SCAN[@]} -eq 0 ]; then
  echo "❌ No repositories to scan. Please check your configuration."
  exit 1
fi
...
extract_dependencies_with_maven() {
  ...
  if [ "$use_reactor" = true ] && [ -n "$relative_path" ]; then
    mvn dependency:tree -DoutputType=text -B -pl "$relative_path" -am > "$TMP_DIR/maven_output_$$.txt" 2>&1 &
  else
    mvn dependency:tree -DoutputType=text -B > "$TMP_DIR/maven_output_$$.txt" 2>&1 &
  fi
  maven_pid=$!
      ...
      if [ ${#REPOS_TO_SCAN[@]} -gt 0 ]; then
        ...
        if [ ${#MAVEN_REPOS_LIST[@]} -gt 0 ]; then
          ...
          for repo in "${MAVEN_REPOS_LIST[@]}"; do
            ...
            if mvn dependency:go-offline; then
              echo ""
