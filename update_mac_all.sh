...
# ==========================
unset_http_proxies() {
  section "Network proxy: unset HTTP(S) 🌐"
  ...
  # Optional cleanup: remove persistent proxy configs for developer tools (commented by default)
  # if has_cmd git;  then run "git config --global --unset http.proxy || true"; run "git config --global --unset https.proxy || true"; fi
  # if has_cmd npm;  then run "npm config delete proxy || true"; run "npm config delete https-proxy || true"; fi
  # if has_cmd yarn; then run "yarn config delete proxy || true"; run "yarn config delete https-proxy || true"; fi
  # Homebrew and curl rely on environment variables only — no persistent proxy settings by default.
}
...
update_intellij_plugins() {
  ...
  if has_cmd idea; then
    ...
    local APP_NAME="IntelliJ IDEA Ultimate.app"
    [[ -d "/Applications/IntelliJ IDEA CE.app" ]] && APP_NAME="IntelliJ IDEA CE.app"
